...
/**
 * SEC-05 FIX: Full HTML entity escaping for values injected into innerHTML.
 * Use escapeHtml() for all Firestore-sourced strings inserted into HTML templates.
 * Unlike sanitizeString(), this does NOT truncate — use for emails, roles, dept names.
 */
...
window.deleteStudent = async function (studentId, name) {
    ...
    try {
        ...
        // MED-08 FIX: Cascade delete ca_question_assignments to prevent orphan records
        try {
            const qaSnap = await window.getDocs(window.query(window.collection(window.db, 'ca_question_assignments'), window.where('studentId', '==', studentId)));
            for (const d of qaSnap.docs) {
                await window.deleteDoc(window.doc(window.db, 'ca_question_assignments', d.id));
            }
        } catch (qaErr) {
            console.warn('Could not cascade delete ca_question_assignments:', qaErr);
        }

        // Delete student
        await window.deleteDoc(window.doc(window.db, 'students', studentId));

        showToast('Student and related records deleted successfully', 'success');
...
async function signup() {
    ...
    // MED-03 FIX: Require department for teachers too, not just HOD/Coordinator.
    // Without department, coordinator-level filtering will exclude the teacher.
    if ((role === 'hod' || role === 'coordinator' || role === 'teacher') && !department) {
        showToast('Please enter department for this role', 'warning');
        return;
    }
    ...
    try {
        ...
        const userData = {
            ...
            // LOW-03 FIX: HOD self-registration is now PENDING (requires approval like coordinators).
            // Auto-approved HOD accounts are a security risk — anyone could create an admin account.
            // HODs must be approved by a super-admin or via the Firebase Console.
...
async function adminCreateUser() {
    // SEC-04 FIX: Re-fetch the caller's role from Firestore instead of trusting window.currentUser.
    // window.currentUser is a plain JS object that can be modified via the browser console.
    // Fetching from Firestore ensures the authorization check cannot be bypassed client-side.
    if (!window.currentUser || !window.auth.currentUser) {
        showToast('Access Denied: Not authenticated.', 'danger');
        return;
    }
    try {
        const callerDoc = await window.getDoc(window.doc(window.db, 'users', window.auth.currentUser.uid));
        if (!callerDoc.exists() || callerDoc.data().role !== 'hod') {
            showToast('Access Denied: Only HOD can create user accounts', 'danger');
            return;
        }
    } catch (e) {
        showToast('Access Denied: Could not verify your role.', 'danger');
        return;
    }
...
<td><span class="badge badge-info">${escapeHtml((data.role || '').toUpperCase())}</span></td>
<td>${extraInfo}</td>
<td>${accountBadge}</td>
<td>${approvalBadge}</td>
<td>${created}</td>
...
${isTeacher ? `<button class="btn btn-sm ${isActive ? 'btn-off' : 'btn-on'}" onclick="toggleTeacherAccount('${docSnap.id}','${escapeHtml(data.email)}',${isActive})">${isActive ? 'Disable' : 'Enable'}</button> ` : ''}
<button class="btn btn-danger btn-sm" onclick="deleteUserFromManage('${docSnap.id}','${escapeHtml(data.email)}')">Delete</button>
</td> `;
        });
...
async function login() {
    ...
    try {
        // BUG-07 FIX: Removed duplicate email/password check that was unreachable dead code.
        // The validation above (pre-try block) already handles this case.
...
// BUG-02 FIX: Declared at module scope so logout() can safely clear it
// without triggering a 'typeof' check workaround.
let _teacherAccountCache = [];
...
function showDashboard(role) {
    ...
    // LOW-04 FIX: Use requestAnimationFrame for theme toggle injection instead of arbitrary timeout
    if (typeof window._injectThemeToggles === 'function') {
        requestAnimationFrame(() => {
            requestAnimationFrame(window._injectThemeToggles);
        });
    }
    // Update Sync UI
...
<td><span class="badge badge-warning">${escapeHtml((data.role || '').toUpperCase())}</span></td>
...
async function rejectUser(userId) {
    ...
    try {
        const userDoc = await window.getDoc(window.doc(window.db, 'users', userId));
        const userData = userDoc.exists() ? userDoc.data() : { email: 'unknown', role: 'unknown' };
        ...
        // BUG-10/MED-10 FIX: Clear user notification and status update
        showToast('User record rejected and deleted. Note: Remove Firebase Auth account from console if required.', "success", 6000);
        loadPendingApprovals();
        loadHODData();
    } catch (error) {
        showToast('Error: ' + error.message, 'danger');
...
async function removeCoordinator(assignmentId, department, email) {
    // MED-05 FIX: Check specific assignmentId rather than generic truthiness to avoid race conditions
    if (window._pendingRemoval !== assignmentId) {
        window._pendingRemoval = assignmentId;
        showToast('Click Remove again to confirm removing ' + email + ' from "' + department + '"', 'warning', 4000);
        setTimeout(() => { if (window._pendingRemoval === assignmentId) window._pendingRemoval = null; }, 4000);
        return;
    }
...
async function loadCoordinatorsList() {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            const row = tbody.insertRow();
            row.innerHTML = `
 <td><strong>${escapeHtml(data.department)}</strong></td>
<td>${escapeHtml(data.coordinatorName || data.email.split('@')[0])}</td>
<td>${escapeHtml(data.email)}</td>
<td>${assignedDate}</td>
<td><button class="btn btn-danger btn-sm" onclick="removeCoordinator('${doc.id}','${escapeHtml(data.department)}','${escapeHtml(data.email)}')">Remove</button></td>`;
        });
...
async function loadAuditLogs() {
    ...
    try {
        ...
        // MED-01 / MED-11 FIX: Removed unused snapshot2 alias
...
// BUG-03 FIX: Replaced sequential N+1 Firestore query loop with parallel batching.
// Old approach: 3 sequential reads per exam = 60+ reads for 20 exams.
// New approach: fetch all subjects + all results in two parallel batches, then
// join data in-memory. Reduces to O(1) round trips regardless of exam count.
async function loadEvaluationProgress(examsSnap) {
    ...
    try {
        const examDocs = examsSnap.docs;

        // Batch-fetch all unique subjects in parallel
        const uniqueSubjectIds = [...new Set(examDocs.map(d => d.data().subjectId).filter(Boolean))];
        const subjectFetches = uniqueSubjectIds.map(sid => window.getDoc(window.doc(window.db, 'subjects', sid)));
        const subjectDocsList = await Promise.all(subjectFetches);
        const subjectMap = {};
        subjectDocsList.forEach(sd => { if (sd.exists()) subjectMap[sd.id] = sd.data(); });

        // Batch-fetch all results + student counts in parallel per exam
        const [resultSnaps, studentSnaps] = await Promise.all([
            Promise.all(examDocs.map(d => window.getDocs(window.query(
                window.collection(window.db, 'results'), window.where('examId', '==', d.id))))),
            Promise.all(examDocs.map(d => {
                const sub = subjectMap[d.data().subjectId];
                if (!sub) return Promise.resolve({ size: 0 });
                return window.getDocs(window.query(window.collection(window.db, 'students'),
                    window.where('class', '==', sub.class),
                    window.where('division', '==', sub.division)));
            }))
        ]);
        ...
        examDocs.forEach((examDoc, idx) => {
            const examData = examDoc.data();
            const subjectData = subjectMap[examData.subjectId] || { name: 'N/A' };
            const resultsSnap = resultSnaps[idx];
            const studentsSnap = studentSnaps[idx];
            ...
            html += `
 <tr>
<td><strong>${sanitizeString(examData.name)}</strong><br><small>${sanitizeString(subjectData.name)}</small></td>
<td>${statusBadge}</td>
...
// BUG-09 FIX: Added missing confirm dialog before subject deletion.
// Also removed the extraneous nested brace wrapper that was dead code (copy-paste artifact).
async function deleteSubject(id) {
    if (!confirm('Are you sure you want to delete this subject? This cannot be undone and will hide it from all views.')) return;
    try {
...
function parseCSV(text) {
    // MED-06 FIX: Warn users that CSV import only supports standard columns (Name, Enrollment, Class, Div)
    showToast('Tip: Use Excel (.xlsx) upload to import extra fields like Batch, Roll No, Email and Phone.', 'info', 5000);
...
async function exportStudentsExcel() {
    try {
        ...
        // MED-07 FIX: Filter export by current Academic Year and Semester if selected
        let queryRef = window.collection(window.db, 'students');
        if (year && semester) {
            queryRef = window.query(queryRef, window.where('academicYear', '==', year), window.where('semester', '==', semester));
        }
        const snapshot = await window.getDocs(queryRef);
        if (snapshot.empty) {
            showToast('No students found to export for selected session', 'warning');
            return;
        }
        const data = [];
        snapshot.forEach(docSnap => {
        ...
        exportToExcel(data, `students_${year || 'all'}_${semester || 'all'}_${Date.now()}`, 'Students');
...
async function loadStudentsList() {
    ...
    try {
        ...
        if (countDiv) {
            ...
            // BUG-08 FIX: Check against 1000 (the actual limit used in the query above),
            // not 500, to correctly warn users when the result set is truncated.
            if (snapshot.size >= 1000) {
                countDiv.textContent += ' (showing first 1000 — use filters to narrow results)';
            }
        }
    } catch (error) {
...
async function createExamUnified(type) {
    ...
    try {
        ...
        const subjectDoc = await window.getDoc(window.doc(window.db, 'subjects', subjectId));
        // BUG-01 FIX: Guard against missing subject document before calling .data()
        if (!subjectDoc.exists()) {
            window.hideLoadingMessage();
            showToast('Subject not found. Cannot assign questions.', 'danger');
            return;
        }
...
async function saveStudentEvaluation(studentId, examId, examType) {
    try {
        ...
        // BUG-01 FIX: Guard against missing subject document before calling .data()
        if (!subjectDoc.exists()) {
            showToast('Error: Subject record not found. Cannot save marks.', 'danger');
            throw new Error('Subject not found: ' + examData.subjectId);
        }
        const subjectData = subjectDoc.data();
...
async function loadEvaluationForm() {
    ...
    try {
        ...
        html += `<div style="padding:20px;text-align:center;">
                    <!-- BUG-05 FIX: Use filteredStudentDocs.length (batch-filtered count) instead of
                         studentsSnap.size (total enrolled count) to show the accurate number of
                         students who will actually be saved in this evaluation pass. -->
                    <button class="btn btn-success btn-lg" onclick="saveAllEvaluations('${examId}','${examData.examType}')">Save All ${filteredStudentDocs.length} Students</button>
                </div>`;
...
// BUG-04 FIX: Replaced deeply nested N+1 sequential queries with parallel batch fetching.
// Old: classes × divisions × 2 sequential reads = ~30 reads for 5 classes, 3 divisions each.
// New: 3 parallel batch reads (all divisions, all batches, all students) then in-memory join.
async function loadClassesList() {
    ...
    try {
        ...
        const activeClasses = classesSnap.docs.filter(d => !d.data().isDeleted);
        const classCodes = [...new Set(activeClasses.map(d => d.data().code))];
        const classIds = activeClasses.map(d => d.id);

        // Batch-fetch all divisions, batches, students in parallel
        const [allDivisionsSnap, allBatchesSnap, allStudentsSnap] = await Promise.all([
            window.getDocs(window.collection(window.db, 'divisions')),
            window.getDocs(window.collection(window.db, 'batches')),
            window.getDocs(window.collection(window.db, 'students'))
        ]);

        // Build lookup maps
        const divisionsByClass = {};
        allDivisionsSnap.docs.forEach(d => {
            const data = d.data();
            if (data.isDeleted) return;
            if (!divisionsByClass[data.classId]) divisionsByClass[data.classId] = [];
            divisionsByClass[data.classId].push({ id: d.id, ...data });
        });

        const batchesByDivision = {};
        allBatchesSnap.docs.forEach(d => {
            const data = d.data();
            if (data.isDeleted) return;
            if (!batchesByDivision[data.divisionId]) batchesByDivision[data.divisionId] = [];
            batchesByDivision[data.divisionId].push(data.name);
        });

        // Count students by class code and by class+division
        const studentsByClass = {};
        const studentsByDiv = {};
        allStudentsSnap.docs.forEach(d => {
            const data = d.data();
            const cls = data.class || '';
            const divKey = `${cls}|${data.division || ''}`;
            studentsByClass[cls] = (studentsByClass[cls] || 0) + 1;
            studentsByDiv[divKey] = (studentsByDiv[divKey] || 0) + 1;
        });

        let html = '';
        for (const classDoc of activeClasses) {
            const classData = classDoc.data();
            const divs = divisionsByClass[classDoc.id] || [];
            const totalStudents = studentsByClass[classData.code] || 0;

            html += `
 <div class="card" style="background: #f9fafb; margin-bottom: 16px;">
<h4 style="color: var(--primary); margin-bottom: 12px;"> ${escapeHtml(classData.code)} - ${sanitizeString(classData.name)}
 <span class="badge badge-info" style="margin-left: 8px;">${totalStudents} students</span>
</h4>
...
<strong>Divisions:</strong> `;
            ...
            if (divs.length === 0) {
                ...
                divs.forEach(divData => {
                    const batchNames = (batchesByDivision[divData.id] || []).sort().join(', ') || '-';
                    const divStudentCount = studentsByDiv[`${classData.code}|${divData.name}`] || 0;
                    html += `
 <tr>
<td><strong>${escapeHtml(divData.name)}</strong></td>
<td><span class="badge badge-secondary">${escapeHtml(batchNames)}</span></td>
<td>${escapeHtml(divData.classTeacher || '-')}</td>
<td><span class="badge badge-success">${divStudentCount}</span></td>
</tr> `;
                });
                html += '</tbody></table>';
            }
...
// LOW-02 FIX: Cleaned up deprecated downloadCSV
...
async function deleteUserFromManage(userId, email) {
    ...
    try {
        ...
        // BUG-10/MED-10 FIX: Clear user notification and status update
        showToast('User record deleted from database. Please also remove the Firebase Auth account in Console.', 'success', 6000);
        loadAllUsers();
        loadHODData();
    } catch (error) {
...
_teacherAccountCache = []; // { id, name, email, isActive, examRestricted, dept }
...
window.printAllMarksheets = async function () {
    ...
    try {
        ...
        const examDoc = await window.getDoc(window.doc(window.db, "exams", examId));
        // BUG-01 FIX: Guard against missing exam document
        if (!examDoc.exists()) {
            showToast('Exam not found', 'danger');
            return;
        }
        ...
        const subjectDoc = await window.getDoc(window.doc(window.db, "subjects", examData.subjectId));
        // BUG-01 FIX: Guard against missing subject document
        if (!subjectDoc.exists()) {
            showToast('Subject record not found', 'danger');
            return;
        }
        const subjectData = subjectDoc.data();
...
async function importFacultyFromExcel() {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        try {
            ...
            // SEC-03 FIX: Use a cryptographically random password instead of the phone number.
            // Phone numbers are publicly known and trivially guessable as passwords.
            // Generate a 16-char random password; coordinator must send a reset email.
            const randomBytes = new Uint8Array(12);
            crypto.getRandomValues(randomBytes);
            const tempPassword = Array.from(randomBytes).map(b => b.toString(16).padStart(2, '0')).join('').substring(0, 16);
            const userCredential = await window.createUserWithEmailAndPassword(authToUse, faculty.email, tempPassword);
            const newUser = userCredential.user;
...
// MED-09 FIX: Configurable CO Rubric with Firestore override support
let CO_RUBRIC = {
...
window.CO_RUBRIC = CO_RUBRIC;

let CO_MAX_MARKS = { CO1: 10, CO2: 10, CO3: 10, CO4: 10, CO5: 10 };
window.CO_MAX_MARKS = CO_MAX_MARKS;

async function loadCORubricConfig() {
    try {
        const rubricDoc = await window.getDoc(window.doc(window.db, 'settings', 'co_rubric'));
        if (rubricDoc.exists()) {
            const data = rubricDoc.data();
            if (data.rubric) {
                CO_RUBRIC = data.rubric;
                window.CO_RUBRIC = CO_RUBRIC;
            }
            if (data.maxMarks) {
                CO_MAX_MARKS = data.maxMarks;
                window.CO_MAX_MARKS = CO_MAX_MARKS;
            }
        }
    } catch (e) {
        console.warn('Using default CO Rubric:', e);
    }
}
window.loadCORubricConfig = loadCORubricConfig;
...
function exportCOAttainmentPDF(role) {
    ...
    // LOW-05 FIX: Guard against popup blocker
    if (!win || win.closed || typeof win.closed === 'undefined') {
        showToast('Popup blocker detected. Please allow popups for this site to export PDF.', 'warning', 6000);
        return;
    }
...
// BUG-06 FIX: Signal that all global app functions are registered and app is ready
if (typeof window._notifyAppReady === 'function') {
    window._notifyAppReady();
}
